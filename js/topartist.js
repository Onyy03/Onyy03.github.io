...
async function loadTopArtists() {
    try {
        ...
        allArtists.forEach(artist => {
            ...
            artist.imageUrl = artist.image || "placeholder.jpg"; 
        });
...
function displayNextArtists() {
    ...
    displayedArtists += nextArtists.length;
}
...
function initTopArtists() {
    loadTopArtists().then(() => {
        ...
        if (loadMoreBtn) {
            loadMoreBtn.addEventListener("click", displayNextArtists);
        }
    });
}
...
async function displayTop3Artists() {
    ...
    artistResponses.forEach((artistData, i) => {
        ...
        artistContainers[i].innerHTML = `
            ...
            <img class="artist-image" src="${artistData.images[0]?.url || 'placeholder.jpg'}" alt="${artist.name}">
            <h3>${artist.name}</h3>
